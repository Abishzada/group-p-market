...
const ProductsDetail = () => {
  const { isProductExists } = useSelector((state) => state.postAdded);
    ...
    return (
      <div key={product?.id}>
        <Row style={{ padding: 50 }}>
          ...
          <Col span={14}>
            <h4 style={{ fontSize: 35, fontWeight: "bolder" }}>
              {product?.category}
            </h4>
            <h1 style={{ fontSize: 45, display: "inline-block" }}>
              {product?.title}
            </h1>
            <p>
              Rating {product?.rating && product?.rating.rate} <StarFilled />
            </p>
            <h3 style={{ fontWeight: "bolder", fontSize: 35 }}>
              {" "}
              $ {product?.price}
            </h3>
            ...
            >
              <Button type="primary" onClick={() => addProduct(product)} block>
                {!isProductExists ? cartBtn : "Add to cart"}
              </Button>
            </Space>
          </Col>
        </Row>
